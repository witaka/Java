...
public class MusicLibrary {
    ...
    private static void createDatabaseTables() throws SQLException
    {
        ...
        { /* Do nothing - valid for non-existent table */ }
    ...
    {
      //  String strGenre = "SELECT  count(`compact_disc`.`compact_disc_id`)\n" +
//"FROM `compact_disc`";
        String strGenre ="";
        ...
        int iCdCount = displayGenreCdCount("SELECT (`compact_disc`.`compact_disc_id`)\n" +
                                           "FROM `compact_disc`");
        int iGenreCount = displayGenreCdCount("SELECT (`genre`.`genre_id`)\n" +
                                              "FROM `genre`");
    ...
    \*------------------------------------------------------------------------*/
    private static int displayGenreCdCount(String strGenre) throws SQLException
    {
        // ToDo - write code/SQL to determine/set iCount for specified genre
        Statement stmt = sm_dbConnection.createStatement();
        ResultSet rs = stmt.executeQuery(strGenre);
        int iCount = 0;
        while (rs.next()) 
        {
        iCount++;
    ...
    private static void displayGenreCdDetails(String strGenre) throws SQLException
    {
        ...
        String strSql = "SELECT *\n" +
                        "FROM  compact_disc\n" +
                        "INNER JOIN  genre ON genre.genre_id = compact_disc.genre_id";
